...
CREATE TABLE child_group (
  ...
  PRIMARY KEY (id, kindergarten_id),
...
CREATE TABLE worker (
  ...
  child_group_kindergarten_id INT NOT NULL,
  status VARCHAR(20) NOT NULL,
  CONSTRAINT fk_worker_child_group1
    FOREIGN KEY (child_group_id , child_group_kindergarten_id)
    REFERENCES child_group (id , kindergarten_id)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
...
CREATE INDEX index_worker ON worker (child_group_id ASC, child_group_kindergarten_id ASC);
CREATE INDEX index_child ON  child (child_group_id ASC, child_group_kindergarten_id ASC);
